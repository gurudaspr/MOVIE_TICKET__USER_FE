...
export default function Shows() {
  ...
  const filterAndSortShowTimes = (showTimes) => {
    showTimes.sort((a, b) => {
      const [hourA, minuteA] = a.showTime.split(':').map(Number);
      const [hourB, minuteB] = b.showTime.split(':').map(Number);
      if (hourA !== hourB) {
        return hourA - hourB;
      } else {
    ...
    return showTimes.map(({ showTime, showId }) => ({
      showTime,
      showId
    }));
  };
        ...
        {shows.map((show) => (
          <li key={`${show.theater}-${show.movieTitle}`} className="py-4">
            ...
            <div className="flex flex-wrap">
              {filterAndSortShowTimes(show.showTimes).map(({ showTime, showId }) => (
                <button
